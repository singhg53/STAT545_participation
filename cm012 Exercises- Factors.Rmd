---
title: "cm012 Exercises: Factors.Rmd"
author: "Gurjot Singh"
date: "11/10/2018"
...
head(iris)
iris %>% 
  mutate(Species = ifelse(Species == "versicolor", "vers", Species)) %>% str()
```
...
- with `rbind()` #base R
- with `bind_rows()` #tidyverse

Which one is more lenient? Which would you prefer?

```{r}
rbind(lotr1, lotr2) #doesn't give any warning and just binds the columns
...
ggplot(gap_small, aes(continent)) +
  geom_bar() +
  scale_x_discrete(drop=FALSE) #what does this do?
```
...
```{r, fig.width=2, fig.height=1}
gap_asia_2007 %>% 
  mutate(country = fct_recorder(country, lifeExp)) %>% 
...
```{r, fig.width=2, fig.height=1}
gap_select %>% 
  mutate(country = forcats::fct_reorder2(country, year,  lifeExp)) %>% 
