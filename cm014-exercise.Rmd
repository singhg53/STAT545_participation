...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
    geom_point() +
    geom_smooth(method="lm") + #this will fit the model
    scale_x_log10()
```
...
```{r}
tidy(my_lm) #use this to interpret how these variables relate to each other
my_lm %>% augment(my_lm, newdata = tibble(gdpPercap = c(400, 500, 600)))
gof <- glance(my_lm)
gof$p.value
```
