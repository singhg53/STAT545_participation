...
Try the same thing, but with `transmute` (drops all other
    variables).
...
It doesn’t make sense to keep those columns because there would be many
values for the population, etc..
...
\#gapminder %\>% mutate(hvsl = if\_else(lifeExp \> 60, “high”, “low”))
group\_by(continent, lifeExp \> 60) %\>% group\_by(continent, hvsl) %\>%
summarize(md = median(gdpPercap)) %\>% arrange(md) \`\`\`
...
cda \<- gapminder %\>% mutate(gdp = gdpPercap \* pop) %\>%
filter(country == “Canada”) %\>% `[[`(“gdp”)

gapminder%\>% mutate(cda\_gdp = rep(cda, nlevels(country)), gdp =
gdpPercap \* pop, rel\_gdp = gdp/cda\_gdp) \`\`\`
...
![](cm008-exercise_files/figure-gfm/pressure-1.png)<!-- -->
